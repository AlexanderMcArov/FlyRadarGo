...
router.get("/area/:latmin/:lonmin/:latmax/:lonmax", async (req,res)=>{
  ...
  try{
  const areaRes = await axios.get(`https://${process.env.OPENSKY_USER}:${process.env.OPENSKY_PASSWORD}@opensky-network.org/api/states/all?lamin=${latmin}&lomin=${lonmin}&lamax=${latmax}&lomax=${lonmax}`)
